...
// -------- Публично: проверка для Tampermonkey --------
app.get('/api/license/verify', async (req, res) => {
  try {
    const { key, device } = req.query;
    if (!key) return res.json({ ok:false, reason:'missing_key' });
    const { rows } = await pool.query('SELECT key, plan, expires_at, is_active, device FROM licenses WHERE key=$1 LIMIT 1', [key]);
    const row = rows[0];
    if (!row) return res.json({ ok:false, reason:'not_found' });
    if (!row.is_active) return res.json({ ok:false, reason:'revoked' });
    if (row.device && device && row.device !== device) return res.json({ ok:false, reason:'bound_to_other_device' });
    const ok = Date.now() < new Date(row.expires_at).getTime();
    return res.json({ ok, plan: row.plan, expiresAt: row.expires_at });
  } catch (e) {
    console.error(e);
...
// -------- Админ‑ручки (для локалки/Render; добавим токен позже) --------
